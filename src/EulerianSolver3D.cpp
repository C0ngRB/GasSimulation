...
void EulerianSolver3D::extractColumnIntegralXY(std::vector<float>& outGrid, float& outMax) const
{
    ...
    for (int j = 0; j < grid_.Ny; ++j) {
        for (int i = 0; i < grid_.Nx; ++i) {
            float acc = 0.0f;
            for (int k = 0; k < grid_.Nz; ++k) {
                const std::size_t id3 = grid_.idx(i, j, k);
                if (!solid_[id3]) {
                    acc += C_[id3] * dz;
                }
            }
            const std::size_t id2 = static_cast<std::size_t>(i) +
                                    static_cast<std::size_t>(j) * static_cast<std::size_t>(grid_.Nx);
            outGrid[id2] = acc;
            outMax = std::max(outMax, acc);
        }
    }
}
